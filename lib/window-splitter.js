...
module.exports = function windowChanger (state) {
  return {
    splitCurrentWindow: function splitCurrentWindow (screenId, winConstructor, winParams, axis) {
      ...
      if (!state.isInGrid(focusedWindow.id, screenId)) return // no-op
      return state.splitWindow(screenId, winConstructor, winParams, axis, focusedWindow.id)
    },
    splitWindow: function splitCurrentWindow (screenId, winConstructor, winParams, axis, winId, listeners) {
      const pane = getPane(state.grids, winId)
      if (!pane) return // no-op
