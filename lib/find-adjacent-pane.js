...
module.exports = {
  findAdjacentPane: function findAdjacentPane (pane, direction, targetGrid) {
  ...
  fuzzyFindAdjacentPane: function fuzzyFindAdjacentPane (pane, direction, targetGrid) {
    const grid = targetGrid || pane.grid
    const candidates = grid.panes
      .filter(p => p.id !== pane.id)
      .reduce((memo, p) => {
        if (direction === 'up' || direction === 'down') {
          if (occupySameVerticalSpace(pane, p)) {
            memo.sameAxisSpace.push(p)
          } else {
            memo.notSameAxisSpace.push(p)
          }
        } else {
          if (occupySameHorizontalSpace(pane, p)) {
            memo.sameAxisSpace.push(p)
          } else {
            memo.notSameAxisSpace.push(p)
          }
    ...
    const choices = candidates.sameAxisSpace.length > 0 ? candidates.sameAxisSpace : candidates.notSameAxisSpace
